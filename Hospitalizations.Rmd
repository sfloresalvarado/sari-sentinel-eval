---
title: "Covid-19 hospitalizations in Chile"
...
load("data/dat_clean hospitalizaciones para publicar.RData")
...
#dat####
d.plot <- dat_clean %>% 
  group_by(semana_anno_sint, anno_sintomas, 
           vigilancia) %>% 
...
d.plot %>% 
  ...
  labs(title = "Incidencia de hospitalizaciones por COVID-19, marzo 2020 - marzo 2023",
       x = "Fecha",
...
pred_m3 <- predict(m3, type = "response", se.fit = T)
...
tab.models %>% 
  filter(Vars %in% c("semana_anno_sint", 
                     "sexoMasculino", "tasa_irag", "positividad", 
                     "Alpha", "B.1.1.348", "B.1.1", "Delta", "Gamma", 
                     "Lambda", "Mu", "N.4", "Omicron", "Otras")) %>% 
...
#dat####
d.plot <- d.mod %>% 
  group_by(semana_anno_sint, anno_sintomas, modelo_n) %>% 
  ...
  mutate(tasa_u = (hospitalizaciones/poblacion) *1e5,
...
d.plot %>% 
  ...
  ggplot(aes(x = semana_anno_sint)) +
  geom_bar(data = . %>% filter(modelo_n == "Modelo 1: básico"), 
  ...
  geom_line(data = . %>% filter(modelo_n == "Modelo 1: básico"), 
            aes(y = tasa_u, color = "Tasa observada\n(hospitalizaciones)"), size = .5) +
...
#dat####
d.plot <- d.mod %>% 
  group_by(semana_anno_sint, anno_sintomas, region_est, modelo_n) %>% 
  summarise(hospitalizaciones = sum(hospitalizaciones),
            pred = sum(pred),
            ub = sum(ub),
            lb = sum(lb)) %>% 
...
d.plot %>%
  ...
  scale_y_continuous(name = "Incidencia estimada por 100.000 habitantes", 
                     sec.axis = sec_axis(~./scale.factor, 
                                         name = "Positividad PCR SARS-CoV-2")) +
  labs(title = "Incidencia de hospitalizaciones por COVID-19, marzo 2020 - marzo 2023, por región",
       x = "Fecha",
       color = "Serie") +
  theme_minimal(base_size = 8) +
...
edad <- levels(d.mod$edad_cat)

d.plot <- d.mod %>% 
  group_by(semana_anno_sint, anno_sintomas, edad_cat, modelo_n) %>% 
  summarise(hospitalizaciones = sum(hospitalizaciones),
            pred = sum(pred),
            ub = sum(ub),
            lb = sum(lb)) %>% 
  ...
  left_join(pob_reg %>%
            filter(edad_cat %in% edad) %>% 
            mutate(edad_cat = as.ordered(edad_cat)) %>% 
            group_by(edad_cat, anno_sintomas) %>% 
            summarise(poblacion = sum(poblacion, na.rm = T))) %>% 
...
d.plot %>% 
  ...
  scale_y_continuous(name = "Incidencia estimada por 100.000 habitantes", 
                     sec.axis = sec_axis(~./scale.factor, 
                                         name = "Positividad PCR SARS-CoV-2")) +
  labs(title = "Incidencia de hospitalizaciones por COVID-19, marzo 2020 - marzo 2023, por edad",
       x = "Fecha",
       color = "Serie") +
  theme_minimal(base_size = 8) +
  facet_wrap("edad_cat", ncol = 2, scales = "free_y")
...
#dat####
d.plot <- d.mod %>% 
  group_by(semana_anno_sint, anno_sintomas, region_est, edad_cat, modelo_n) %>% 
  summarise(hospitalizaciones = sum(hospitalizaciones),
            pred = sum(pred),
            ub = sum(ub),
            lb = sum(lb)) %>% 
  ...
  left_join(pob_reg %>%
            filter(edad_cat %in% edad) %>% 
            mutate(edad_cat = as.ordered(edad_cat)) %>% 
            group_by(edad_cat, region_est, anno_sintomas) %>% 
            summarise(poblacion = sum(poblacion, na.rm = T))) %>% 
...
d.plot %>% 
  ...
  scale_y_continuous(name = "Incidencia estimada por 100.000 habitantes", 
                     sec.axis = sec_axis(~./scale.factor, 
                                         name = "Positividad PCR SARS-CoV-2")) +
  labs(title = "Incidencia de hospitalizaciones por COVID-19, marzo 2020 - marzo 2023, por región y edad",
       x = "Fecha",
       color = "Serie") +
  theme_minimal(base_size = 8) +
...
g1 <- d.plot %>% 
  ggplot(aes(x = semana_anno_sint)) +
  geom_line(aes(y = tasa_u, linetype = "Observed hospitalized cases rate"), size = .5) +
  labs(title = "Assessment of errors in the hospitalization incidence estimation, March 2020 - March 2023",
       x = "Date", 
...
d.plot %>% 
  ggplot(aes(x = semana_anno_sint)) +
  geom_line(aes(y = abs(se), color = modelo_n), size = .5) +
  geom_hline(yintercept = .1, linetype = 3) +
  labs(x = " ",
       y = "Standard error",
       color = "Model") +
...
g4 <- d.plot %>% 
  ggplot(aes(x = semana_anno_sint)) +
  geom_line(aes(y = ecm, color = modelo_n), size = .5) +
  geom_hline(yintercept = .1, linetype = 3) +
  labs(x = "Date",
       y = "Mean square error\n",
       color = "Model") +
...
d.plot %>% 
  ...
  labs(title = "Bland-Altman plot for the hospitalizations national incidence estimation, March 2020 - March 2023",
...
d.pred <- dat_clean %>% 
  ...
  mutate(tasa_irag = (irag/poblacion_irag) * 1e5,
         ...
         prop_vac_2da = replace_na(prop_vac_2da, 0),
         prop_vac_ref = replace_na(prop_vac_ref, 0),
         hospitalizaciones = replace_na(hospitalizaciones, 0) ) %>% 
  dplyr::select(-irag, -poblacion_irag) %>% 
  mutate(across(Alpha:Omicron, replace_na, 0)) %>% 
  group_by(edad_cat, sexo, semana_anno_sint) %>% 
  fill(tasa_irag, .direction = "downup") %>% 
  group_by(edad_cat, semana_anno_sint) %>% 
  fill(tasa_irag, .direction = "downup") %>% 
  group_by(semana_anno_sint) %>% 
  fill(tasa_irag, .direction = "downup") %>% 
  ungroup() %>% 
...
```{r}
pred_nac <- predict(m3, 
                    newdata = d.pred %>% mutate(region_est = as.character(region_est)), 
                    type = "response", 
                    se.fit = T,
...
d.plot <- d.pred %>% 
  ...
  summarise(hospitalizaciones = sum(hospitalizaciones),
            ...
            ub = sum(ub),
            lb = sum(lb),
            ic_cob = ifelse((hospitalizaciones <= ub) & (hospitalizaciones >= lb), T, F),
        ) %>% 
  left_join(pob_reg %>% 
              group_by(anno_sintomas) %>% 
              summarise(poblacion = sum(poblacion, na.rm = T))) %>% 
...
d.plot %>% 
  ...
  geom_line(aes(y = hospitalizaciones, color = "Casos observados\n(hospitalizaciones)"), size = .5) +
...
d.pred %>% 
  ...
  rbind(d.pred %>% 
          ...
          summarise(hospitalizaciones = sum(hospitalizaciones),
            ...
            lb = sum(lb)) %>% 
          ungroup() ) %>% 
  mutate_at(c("hospitalizaciones", "pred", "ub", "lb"), floor) %>% 
  mutate(anno = replace_na(as.character(anno_sintomas), "Total"),
         .keep = "unused",
         .before = modelo_n) %>% 
  kable(caption = "Hospitalizaciones observadas y predichas por año") %>%
  kable_paper(full_width = F, 
              bootstrap_options = "striped", 
              fixed_thead = T) %>% 
