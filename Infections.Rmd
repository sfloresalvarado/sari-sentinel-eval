...
load("data/dat_clean contagios.RData")
```
...
d.plot <- dat_clean %>% 
  ...
  #mutate(conteo_mean = rollapply(conteo, 4, mean, align = "center", fill = NA, partial = T),
   #      conteo_mean = floor(conteo_mean)) %>% 
  ungroup() %>% 
  mutate(poblacion = ifelse(vigilancia == "irag", poblacion_irag, poblacion),
         ...
         #corrección: por proporción de hospitalizaciones
         #irag no sirve para estimar incidencia de contagios sino de
         #hospitalizaciones, salvo que apliquemos esta corrección

#plot####
d.plot %>% 
  ggplot(aes(x = semana_anno_sint)) +
  geom_line(aes(y = tasa_est, color = vigilancia)) +
 # geom_line(aes(y = tasa_est, color = vigilancia), linetype = 3) +
...
load("modelos OE1 contagios.RData")
...
####
```{r juntar datos modelos, eval=F}
d.mod <- rbind(d.mod0, d.mod1, d.mod2, d.mod3)
rm(d.mod0, d.mod1, d.mod2, d.mod3)

mods <- list(m0, m1, m2, m3)

save(d.mod, file = "modelos datos OE1 contagios.RData")
save(mods, file = "modelos OE1 contagios.RData")
```
...
d.plot %>%
  filter(modelo_n != "Modelo 0: nulo") %>% 
  #filter(region_est %in% c("La Araucanía", "Magallanes")) %>% 
  ggplot(aes(x = semana_anno_sint)) +
  geom_bar(data = . %>% filter(modelo_n == "Modelo 3: final"), 
            aes(y = positividad*scale.factor), 
           stat = "identity", position = "identity", width = 5, alpha = .15) +
...
#peaks muy sobreestimados en magallanes y la araucanía
...
#peak de omicrón muy sobreestimado en grupo [70,80)
  
ggsave("Plots/OE1 incidencia estimada x reg + edad.png",
       height = 10,
       width = 20,
       bg = "white")
...
#         mape = abs(tasa_u - tasa_pred)/abs(tasa_u),
         ecm = sesgo^2 + se^2,
...
g1 <- d.plot %>% 
  ggplot(aes(x = semana_anno_sint)) +
#  geom_line(aes(y = tasa_i*scale.factor1, linetype = "b. Tasa estimada\n(IRAG)"), size = .5) +
  geom_line(aes(y = tasa_u, linetype = "Observed total cases rate"), size = .5) +
#  scale_y_continuous(name = "Incidencia notificación universal\n", 
#                     sec.axis = sec_axis(~./scale.factor1, 
#                                         name = "Incidencia estimada por IRAG")) +
...
#            aes(y = sesgo_rel_i, color = "Tasa estimada\n(IRAG)"), size = .5) +
...
#scale.factor2 <- (max(d.plot$sesgo_se, na.rm = T) / max(d.plot$sesgo_se_i, na.rm = T))

g3 <- d.plot %>% 
  ggplot(aes(x = semana_anno_sint)) +
  geom_line(aes(y = abs(sesgo_se), color = modelo_n), size = .5) +
#  geom_line(aes(y = sesgo_se_i*scale.factor2, color = "Tasa estimada\n(IRAG)"), size = .5) +
  geom_hline(yintercept = .1, linetype = 3) +
#  scale_y_continuous(name = "\nSesgo/SE\n", 
#                     sec.axis = sec_axis(~./scale.factor2, 
#                                         name = "Sesgo/SE tasa IRAG")) +
  labs(x = " ",
       y = "Standardized bias\n",
       color = "Model") +
  theme_minimal(base_size = 8)
...
save(d.pred, file = "modelos datos OE1 pred nac contagios.RData")

rm(pred_nac, d.pred)
```

```{r}
load("modelos datos OE1 pred nac contagios.RData")
```
